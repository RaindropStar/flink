...
public class KeydProcessFunctionTopNDemo {
    public static void main(String[] args) throws Exception {
        ...
        /**
         * 1.按照vc坐keyby，开窗，分别count
         *     增量聚合，计算count
         *     全窗口，对计算结果count值封装，带上窗口结束的时间
         *        为了让同一个窗口时间范围的计算结果到一起去
         * 2.对同一个窗口范围的count值进行处理： 排序、取前N个
         *      按照windowEnd做keyby
         *      使用process，来一条调用一次，需要先存，分开存HashMap，key=windowsEnd，value=list
         */

        //1. 按照vc分组，开窗，聚合（增量计算+全量打标签）
        //开窗聚合后变成普通的流，没有了窗口信息，需要自己打上窗口的标记
        ...
        //2. 按照窗口标签（窗口结束时间）keyby，保证同一个窗口时间范围的结果到一起去，排序，取topN
    ...
    public static class WindowResult extends ProcessWindowFunction<Integer, Tuple3<Integer,Integer,Long>,Integer,TimeWindow> {
         ...
         * 第二个： 输出类型= Tuple3（vc,count,WindowEnd）,带上窗口的结束标签
         * 第三个： key类型，vc Interger
         * 第四个： 窗口类型
         * @param context
         * @param iterable
         * @param collector
    ...
    public static class TopN extends KeyedProcessFunction<Long,Tuple3<Integer, Integer, Long>,String>{
        //存不同窗口 统计结果，key=windowEnd，value =list数据
        ...
        public void processElement(Tuple3<Integer, Integer, Long> integerIntegerLongTuple3, KeyedProcessFunction<Long, Tuple3<Integer, Integer, Long>, String>.Context context, Collector<String> collector) throws Exception {
            ...
            if (dataListMap.containsKey(windowEnd)) {
                ...
                ArrayList<Tuple3<Integer, Integer, Long>> dataList = new ArrayList<>();
                dataList.add(integerIntegerLongTuple3);
                dataListMap.put(windowEnd,dataList);
            }

            //2.注册一个定时器，windowEnd + 1ms即可（同一个窗口范围，应该同时输出）
            //只不过是一条一条调用processElement方法，只需要延迟1ms即可
        ...
        public void onTimer(long timestamp, KeyedProcessFunction<Long, Tuple3<Integer, Integer, Long>, String>.OnTimerContext ctx, Collector<String> out) throws Exception {
            ...
            //定时器触发，同一个窗口范围的计算结果攒齐，开始排序，取TopN
            ...
            //遍历 排序后的list，取出前n(thershold)个，避免list不够两个， 元素个数 和 2 取最小值
            outStr.append("======================");
